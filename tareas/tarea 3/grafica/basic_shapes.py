...
def createColorTriangleEsc(r, g, b):
...
def readOFFTexture(filename):
    ...
    with open(filename, 'r') as file:
        ...
        for vertex in vertices:
            ...
            v = vertex[2] / np.max(vertices[:, 2])
            texture_coords.append([u, v])

        texture_coords = np.asarray(texture_coords)

        vertexData = np.concatenate((vertices, texture_coords), axis=1)
...
def createTextureCylinder():
    return readOFFTexture(getAssetPath('cylinder.off'))